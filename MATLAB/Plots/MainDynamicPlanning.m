...
filename = 'Data/PathRecalc2_3/adaptive_7.txt';
[x, y] = extractPath(filename);

% Load Planning Points
%planning_points = load('Data/PathPropagationRRT/adaptive_flag_3.txt');
planning_points = load('Data/PathRecalc2_3/adaptive_flag_7.txt');
planning_points(end+1, :) = [x{end}(end), y{end}(end)];

% Load Obstacles
obstacles = load('Data/PathRecalc2_3/obstacles_7.txt');

% Load Intermediate Goal Index
intGoal_idex = load('Data/PathRecalc2_3/adaptive_goalIndex_7.txt');
...
for i = 1:length(x)+1
    ...
    elseif i <= length(x)
        % Plot agent location
        scatter(x{i}(divergence_index(i-1)), y{i}(divergence_index(i-1)), agent_marker_size, 'filled', ...
                'MarkerFaceColor', agent_color, ...
                'MarkerEdgeColor', 'k', 'DisplayName', 'Agent', 'HandleVisibility', 'off');
        ...
        % Plot planning section
        plot(x{i}(planning_index(i-1):divergence_index(i-1)),y{i}(planning_index(i-1):divergence_index(i-1)),...
            'LineWidth', 2,'Color', 'm','DisplayName','Planning');
        % Plot New Path
        new_index = find(x{i-1}(intGoal_idex(i)) ==x{i});
        plot(x{i}(divergence_index(i-1):new_index), y{i}(divergence_index(i-1):new_index), 'LineWidth', 2, ...
